...
- `[ramcnt-skip] <func> ...` — gated behind `ENABLE_PERF_TELEMETRY`.
  ...
  dereferencing them. The stage-background loader preflights every required
  PPG source (type `0x12`, plus `0x1F` outside bonus stages) before mutating
  render state; `[bg-texture-skip]` means a legitimate cache teardown was
  observed after a required source had already been purged.
